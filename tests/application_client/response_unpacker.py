...
from application_client.transaction import Signature
...
#            chain_code_len (1)
#            chain_code (var)
def unpack_get_public_key_response(
        response: bytes) -> Tuple[int, bytes, int, bytes]:
    response, pub_key_len, pub_key = pop_size_prefixed_buf_from_buf(response)
    response, chain_code_len, chain_code = pop_size_prefixed_buf_from_buf(
        response)
...
# Unpack from response:
# response = signature
def unpack_sign_tx_response(response: bytes) -> Signature:
    return Signature.deserialize(response)
